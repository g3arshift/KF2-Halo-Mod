...
class KFWeap_ScopeTestPistol extends KFWeap_PistolBase;
...
simulated function DrawHUD( HUD H, Canvas C )
{
	...
	if(!TestPistolUI.isInitialized)
	{
		KFPC = KFPlayerController(Instigator.Controller);
		TestPistolUI.InitializeWeaponUI(C, KFPC, UIBackgrounds, Reticle_Neutral, Reticle_Friendly, Reticle_Enemy, Reticle_Headshot, true, FlavorIcons);
	}

	if( bUsingSights )
	{
		TestPistolUI.RunWeaponUI(C);
	}
}
...
defaultproperties
{
	...
	RecoilMaxYawLimit=0
	RecoilMinYawLimit=0
	RecoilMaxPitchLimit=0
	RecoilMinPitchLimit=0
	RecoilISMaxYawLimit=0
	RecoilISMinYawLimit=0
	RecoilISMaxPitchLimit=0
	RecoilISMinPitchLimit=0
